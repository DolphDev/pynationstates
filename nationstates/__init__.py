...
class Api(object):
    ...
    def __init__(self, _type_, value = None, shard = None, limit = None,user_agent=None, args=None):
        ...
        self.collect_data = None #To store the last collect() call

    def __call__(self, _type_, value = None, shard = None, limit=None,  user_agent= None, args=None):

        """
        Handles the arguments and sends the args to be parsed

        Then sets up a NSback.Api instance (api_instance) that the object will interact with

        :param _type_: The type of API being accessed ("nation", "region", "world", "wa")

        :param value: The value of the API type (For the example, the nation to search when using "nation")

        :param shard: A list (preferablly set) of nationstate shards that NSback uses to both request and parse data from nationstates)

        :param limit: The Limit that will be appended to the end of the request if a limit is accepted. May be Depreciated due to the Shard object.

        :param user_agent: A custom useragent if needed. The Nationstates Module has a defualt message if this is left blank
        ...
        self.api_instance = NSback.Api(_type_, value = value, shard = shard, limit = limit, user_agent = None, parse_args = self.parse_args)

    def load(self):
        self.api_instance.load()
        self.collect_data = None #Since the data is new we need 
        ...
        if type(parse_list) is list: 
            for x in args:
                ...
                if x == "censusid":
                    ...
                    tempcall.load()
                    parse_args = NSback.DictMethods.merge_dicts(parse_args, {"censusid":tempcall.collect()["censusid"]})
